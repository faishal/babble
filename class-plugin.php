...
class Babble_Plugin {
	...
	protected function locate_template( $template_file ) {
		...
		if ( file_exists( STYLESHEETPATH . "/$sub_dir" . $template_file ) ) {
			return STYLESHEETPATH . "/$sub_dir" . $template_file; } // If there's a tpl in the parent of the current child theme
		else if ( file_exists( TEMPLATEPATH . "/$sub_dir" . $template_file ) ) {
			return TEMPLATEPATH . "/$sub_dir" . $template_file; } // Fall back on the bundled plugin template (N.B. no filtered subfolder involved)
