<div class="wrap">
	...
	<div>
		<form action="" method="get">
			...
			<p><?php
				...
				$selected_stati = array();
				if ( isset( $_GET[ 'bbl_stati' ] ) && is_array( $_GET[ 'bbl_stati' ] ) ) {
					$selected_stati = array_map( 'sanitize_text_field', $_GET[ 'bbl_stati' ] );
				} else {
					$selected_stati = array(
						'publish',
						'private',
				...
				foreach ( $stati as $status => $status_obj ) : {
					?>
					<label for="status-<?php echo esc_attr( $status ); ?>"><input type="checkbox" name="bbl_stati[]"
					                                                              value="<?php echo esc_attr( $status ); ?>"
					                                                              id="status-<?php echo esc_attr( $status ); ?>" <?php checked( in_array( $status, $selected_stati ) ); ?> /> <?php echo esc_html( $status_obj->label ); ?>
						(<?php echo $status_obj->public ? __( 'public', 'babble' ) : __( 'hidden', 'babble' ); ?>
						)</label><br/>
				<?php } endforeach; ?></p>
	...
	$term_per_page = 10;
	$offset        = ( $page - 1 ) * $term_per_page;
	$page_url           = admin_url( 'tools.php?page=btgt' );
	$terms         = get_terms( 'post_translation', array( 'number' => $term_per_page, 'offset' => $offset, 'orderby'=>'id' , 'order' => 'DESC' ) );
	//paging
		...
		<div class="tablenav-pages"><span class="displaying-num"></span>
			<?php if ( $page > 1 ) { ?>
				<a class="prev-page" title="Go to the previous page"
				   href="<?php echo esc_url(add_query_arg(array("paged" => $page - 1 ), $page_url ) ) ;  ?>">‹ Previous</a>
			<?php } ?>
			<?php if ( count( $terms ) == $term_per_page ) { ?>
				<a class="next-page" title="Go to the next page"
				   href="<?php echo esc_url(add_query_arg(array("paged" => $page + 1 ), $page_url ) ) ; ?>">Next ›</a>
			<?php } ?>
			<br class="clear">
		</div>
	</div>
			...
			<tbody id="tg-<?php echo esc_attr( $term->term_id ); ?>">
				...
				if ( $posts ) : {
					...
					<?php foreach ( $posts as $post ) : {
						if ( !in_array( $post->post_status, $selected_stati ) ) {
							continue;
						} ?>
						<?php if ( !$post ) : { ?>
							<tr>
								<th colspan="4">
									...
									–
									<a href="<?php echo esc_url( $this->get_action_link( $post_id, 'delete_from_groups' ) ); ?>">remove
										from all groups</a>
								</th>
							</tr>
						<?php } else : { ?>
							<tr class="post-id-<?php echo esc_attr( $post->ID ); ?>">
								<th scope="row" class="manage-column column-id">
									...
									<a href="<?php echo esc_url( add_query_arg( array( 'lang' => bbl_get_post_lang_code( $post->ID ) ), get_edit_post_link( $post->ID ) ) ); ?>"><?php esc_html_e( 'edit', 'babble' ); ?></a>
									|
									<a href="<?php echo esc_url( $this->get_action_link( $post->ID, 'delete_post', "tg-$term->term_id" ) ); ?>"><?php esc_html_e( 'delete', 'babble' ); ?></a>
									|
									<a href="<?php echo esc_url( $this->get_action_link( $post->ID, 'trash_post', "tg-$term->term_id" ) ); ?>"><?php esc_html_e( 'trash', 'babble' ); ?></a>
									|
									<?php if ( bbl_get_default_lang_code() == bbl_get_post_lang_code( $post->ID ) ) : { ?>
										<a href="<?php echo esc_url( $this->get_action_link( $post->ID, 'delete_from_groups', "tg-$term->term_id" ) ); ?>">remove
											from group</a>
									<?php } endif; ?>
								</th>
				...
				<?php } else : { ?>
					...
					<tr>
						<td colspan="4"><em>no posts found for this translation group</em></td>
					</tr>
